...
HRESULT CDXUTMesh::RenderTokamak( ID3DXEffect *pEffect,
                           D3DXHANDLE hDiffuse, D3DXVECTOR4* pDiffuse,
                           D3DXHANDLE hAmbient, D3DXVECTOR4* pAmbient,
                           D3DXHANDLE hSpecular, D3DXVECTOR4* pSpecular,
                           D3DXHANDLE hPower, float Power,
                           bool bDrawOpaqueSubsets,
                           bool bDrawAlphaSubsets )
{
    if( NULL == m_pMesh )
    ...
    if( bDrawOpaqueSubsets )
    {
        ...
        for( UINT p = 0; p < cPasses; ++p )
        {
            ...
            for( DWORD i=0; i<m_dwNumMaterials; i++ )
            {
                // D3DCOLORVALUE and D3DXVECTOR4 are data-wise identical.
                // No conversion is needed.
                if( hDiffuse && pDiffuse->w < 1.0f)
					continue;

				if( hDiffuse)
                    pEffect->SetVector( hDiffuse, pDiffuse );
                if( hAmbient )
                    pEffect->SetVector( hAmbient, pAmbient );
                if( hSpecular )
                    pEffect->SetVector( hSpecular, pSpecular );
                if( hPower )
                    pEffect->SetFloat( hPower, Power );
    ...
    if( bDrawAlphaSubsets && m_bUseMaterials )
    {
        ...
        for( UINT p = 0; p < cPasses; ++p )
        {
            ...
            for( DWORD i=0; i<m_dwNumMaterials; i++ )
            {
                if( hDiffuse == NULL || pDiffuse->w == 1.0f)
					continue;
