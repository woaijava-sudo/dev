...
void UIContainer::setManager(UIManager* pManager)
{
    ...
    std::for_each(m_ctrls.begin(), m_ctrls.end(), [&](RefPtr<UIControl> x)
    {
...
bool UIContainer::addControl(UIControl* ctrl)
{
    ASSERT(ctrl);
...
bool UIContainer::removeControl(UIControl* ctrl)
{
    ASSERT(ctrl);
    if (!m_ctrls.empty())
    {
...
void UIContainer::destroy()
{
    std::for_each(m_ctrls.begin(), m_ctrls.end(), [&](RefPtr<UIControl> x)
    {
        getManager()->unregisterControl(x.get());
    });
    m_ctrls.clear();
}
...
{
    std::for_each(m_ctrls.rbegin(), m_ctrls.rend(), [&](RefPtr<UIControl> x)
    {
        x->onPaint(graphics, rect);
    });
}

PassRefPtr<UIControl> UIContainer::FindControl(UI_FIND_CALL proc, void* data, UINT flags)
{
    ...
    if ((flags & UIFIND_TOP_FIRST) != 0) // ÕýÐò
    {
...
PassRefPtr<UIControl> UIContainer::FindControlById(WORD id)
{
    return FindControl(UIManager::__FindControlFromId, (void*)(id), UIFIND_HITTEST | UIFIND_TOP_FIRST);
}

PassRefPtr<UIControl> UIContainer::FindControlLv1(WORD id)
{
    auto ctrl = std::find_if(m_ctrls.begin(), m_ctrls.end(), [&](RefPtr<UIControl> x)
    {
        return x->getId() == id;
    });
...
PassRefPtr<UIControl> UIContainer::FindControlInternal(WORD id)
{
