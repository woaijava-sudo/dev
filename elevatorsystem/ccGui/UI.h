...
typedef PassRefPtr<UIControl>(CALLBACK* UI_FIND_CALL)(PassRefPtr<UIControl>, void*);
...
public:
    ...
    void init();
    bool registerControl(PassRefPtr<UIControl> pCtrl);
    bool unregisterControl(PassRefPtr<UIControl> pCtrl);
    PassRefPtr<UIContainer> getLayout() const { return m_layout; }
    ...
    static PassRefPtr<UIControl> CALLBACK __FindControlFromPoint(PassRefPtr<UIControl>, void*);
    static PassRefPtr<UIControl> CALLBACK __FindControlFromId(PassRefPtr<UIControl>, void*);
    PassRefPtr<UIControl> FindControl(Point pt);
    PassRefPtr<UIControl> FindControl(WORD id, WORD direct);
    ...
    template<typename T>
    void postMessage(const PassRefPtr<T>& ctrl, UINT nCmd, WORD wDirect = UIDFT_CONTROL_DIRECT_ID, WORD wParam = 0)
    {
        postMessage(ctrl.get(), nCmd, wDirect, wParam);
    }
    template<typename T>
    LRESULT sendMessage(const PassRefPtr<T>& ctrl, UINT msg, WPARAM wp = 0, LPARAM lp = 0, UINT flag = 0, Point pt = Point(), bool recursion = false)
    {
        return sendMessage(ctrl.get(), msg, wp, lp, flag, pt, recursion);
    }

    template<typename T>
    PassRefPtr<T> createObject(CRuntimeClass* pClass)
    {
        ASSERT(pClass->IsDerivedFrom(T::GetThisClass()));
        return adoptRef(static_cast<T*>(pClass->CreateObject()));
    }
...
protected:
    template<typename T = UIControl> PassRefPtr<T> getControl(WORD id);
    ...
    template<typename T>
    void postCommandMessage(const PassRefPtr<T>& ctrl, UINT nCmd, WORD wDirect = UIDFT_CONTROL_DIRECT_ID, WORD wParam = 0)
    {
        postCommandMessage(ctrl.get(), nCmd, wDirect, wParam);
    }
    template<typename T>
    LRESULT sendCommandMessage(const PassRefPtr<T>& ctrl, UINT msg, WPARAM wp = 0, LPARAM lp = 0, UINT flag = 0, Point pt = Point(), bool recursion = false)
    {
...
private:
    ...
    CDialogEx* m_dlg;
    CRect m_client;
    RefPtr<UIContainer> m_layout;
    CDC m_bufferedDC;
    CBitmap m_bufferedBMP;
    CDC m_maskDC;
    CBitmap m_maskBMP;
    CMapWordToOb m_register;
};

class UIControl : public CObject, public RefCounted<UIControl>
{
...
public:
    ...
    virtual PassRefPtr<UIControl> FindControl(UI_FIND_CALL proc, void* data, UINT flags);
...
public:
    ...
    template<typename T> bool addControl(const PassRefPtr<T>& ctrl) { return addControl(ctrl.get()); }
    template<typename T> bool removeControl(const PassRefPtr<T>& ctrl) { return removeControl(ctrl.get()); }
    bool addControl(UIControl* ctrl);
    bool removeControl(UIControl* ctrl);

    virtual void onPaint(Graphics& graphics, Rect rect);
    virtual void onPaintChildren(Graphics& graphics, Rect rect);

    virtual PassRefPtr<UIControl> FindControl(UI_FIND_CALL proc, void* data, UINT flags);
    virtual PassRefPtr<UIControl> FindControlById(WORD id);
    virtual PassRefPtr<UIControl> FindControlLv1(WORD id);
    virtual void onRecursion(UINT message);
    virtual void onCommand(DWORD id, UINT message);
...
protected:
    ...
    virtual PassRefPtr<UIControl> FindControlInternal(WORD id);

private:
    std::set<RefPtr<UIControl>> m_ctrls;
};
