...
void UIManager::init()
{
    ...
    CString caption; caption.LoadString(IDS_CAPTION);
...
bool UIManager::registerControl(PassRefPtr<UIControl> ctrl)
{
    if (ctrl->hasAttr(STATE_NOID))
        return false;
    CObject* o;
    if (m_register.Lookup(ctrl->getId(), o))
    {
        return false;
    }
    TRACE("register ctrl: 0x%08X <%s> [%d] '%S'\n", ctrl, ctrl->GetRuntimeClass()->m_lpszClassName, ctrl->getId(), ctrl->getName());
    m_register.SetAt(ctrl->getId(), ctrl.get());
    return true;
}

bool UIManager::unregisterControl(PassRefPtr<UIControl> ctrl)
{
    ...
    if (m_register.Lookup(ctrl->getId(), o))
    {
        ...
        m_register.RemoveKey(ctrl->getId());
        return true;
    }
    return false;
}
...
PassRefPtr<UIControl> CALLBACK UIManager::__FindControlFromPoint(PassRefPtr<UIControl> ctrl, void* data)
{
    return ctrl->getRect().Contains(*static_cast<Point*>(data)) ? ctrl : nullptr;
}

PassRefPtr<UIControl> CALLBACK UIManager::__FindControlFromId(PassRefPtr<UIControl> ctrl, void* data)
{
    return ctrl->getId() == (WORD)(data) ? ctrl : nullptr;
}

PassRefPtr<UIControl> UIManager::FindControl(Point pt)
{
    return m_layout->FindControl(__FindControlFromPoint, &pt, UIFIND_VISIBLE | UIFIND_HITTEST | UIFIND_TOP_FIRST);
}

PassRefPtr<UIControl> UIManager::FindControl(WORD id, WORD direct)
{
    if (direct != UIDFT_CONTROL_DIRECT_ID)
    {
        ...
        else
        {
            ...
            if (ctrl && ctrl->IsKindOf(RUNTIME_CLASS(UIContainer)))
                return static_cast<UIContainer*>(ctrl.get())->FindControlLv1(direct);
        }
        return nullptr;
    }
...
PassRefPtr<T> UIManager::getControl(WORD id)
{
    CObject* o;
    if (m_register.Lookup(id, o))
    {
        ASSERT(o);
        ASSERT(o->IsKindOf(RUNTIME_CLASS(T)));
